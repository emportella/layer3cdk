...
```bash
npm install @emportella/@emportella/layer3cdk
```
...
Add the `@emportella/layer3cdk` context to your `cdk.json` for centralized team and department config:

```json
{
  "context": {
    "@emportella/layer3cdk": {
      "team": "Layer3",
      "department": "pltf"
    }
  }
...
Users can extend (add keys) or override (strip all defaults) via the `tags` section in `@emportella/layer3cdk` context:
