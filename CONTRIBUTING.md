...
- Test your changes locally: `npm run build && npm run lint && npm test`

### Testing in a Real Project
- Once your PR is approved, test the construct in a real CDK project before final merge.
- You can link to your local build using `npm link` or install from a branch build.
...
- Publishing to npm is triggered manually via `workflow_dispatch` by a maintainer.
...
2. Update the version: `npm version <major|minor|patch>`.
3. Update `CHANGELOG.md` following [Keep a Changelog](http://keepachangelog.com/en/1.0.0/).
4. Push with tags: `git push --follow-tags`.
5. A maintainer triggers the **Publish to npm** workflow from the Actions tab.
